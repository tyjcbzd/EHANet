...
if __name__ == "__main__":
    ...
    checkpoint_path = f"Your weight path"
    file = open(f"test_clinicDB_results.csv", "w")
    ...
    """ load datasets """
    (train_x, train_y, train_edge, train_names), (test_x, test_y, test_edge, test_names) = load_dataset(path) 
    """ Hyperparameters """
    size = (256, 256)
    ...
    time_taken = []
    total_score = []
    for i, (x, y,test_name) in tqdm(enumerate(zip(test_x, test_y, test_edge, test_names)), total=len(test_x)):
        ## Image
